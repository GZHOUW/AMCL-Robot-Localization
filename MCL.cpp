#define _USE_MATH_DEFINES // must be defined before iostream
...
public:
    Robot(){
        ...
        forward_noise = 0.00001; //noise of the forward movement
        turn_noise = 0.00001; //noise of the turn
        sense_noise = 0.00001; //noise of the sensing
    }
...
int main() {
    /*
    //Examples of Interfacing with Robot Class
    Robot myrobot;
    myrobot.set_noise(5.0, 0.1, 5.0); // Create turning, moving noise
    myrobot.set(30.0, 50.0, M_PI / 2.0); // Set robot new position and orientation
    myrobot.move(-M_PI / 2.0, 15.0); // Move the robot (will create nosie)
    //cout << myrobot.read_sensors() << endl;
    myrobot.move(-M_PI / 2.0, 10.0);
    //cout << myrobot.read_sensors() << endl;
    */

    // Create a particle array, which has n robot objects
    const int n = 1000;
    Robot p[n]; // 

    // Create new forward_noise, turn_noise, and sense_noise
    for (int i = 0; i < n; i++) {
        p[i].set_noise(0.05, 0.05, 5.0);
    }

    //Initialize myrobot object and a measurment vector, which is distance from the robot to landmarks
    Robot myrobot = Robot();
    vector<double> z;
    ...
    for (int t = 0; t < steps; t++) {
        ...
        z = myrobot.sense(); // distance from the robot to landmarks

        // Create a NEW particle array, which contains the particles AFTER movement, PRIOR to measurement
        Robot p_prior[n];
        for (int i = 0; i < n; i++) {
            p_prior[i] = p[i].move(0.1, 5.0); // move each particle and store its new state
            p[i] = p_prior[i];
        }

        // Use sensor measurement to generate particle weights 
        double weights[n];
        for (int i = 0; i < n; i++) {
            weights[i] = p[i].measurement_prob(z); // Each particle has a weight
        }

        //Resample the particles with a sample probability proportional to the importance weight
        // Create a NEW particle array, which contains the particles AFTER measurement (Posterior X_t)
        Robot p_posterior[n];
        int index = gen_real_random() * n; // random index between 0 and n
        //cout << index << endl;
        double beta = 0.0;
        double max_weight = max(weights, n);
        //cout << mw << endl;
        ...
        for (int i = 0; i < n; i++) {
            ...
            while (beta > weights[index]) {
                beta -= weights[index];
                index = mod((index + 1), n); // move index until it points at the particle with max weight
            }
            p_posterior[i] = p[index]; // set that max-weight particle as the first particle in posterior
        }

        for (int k = 0; k < n; k++) { // update p so that it is equal to posterior
            p[k] = p_posterior[k];
